...
<script>
    ...
    function lastPage(pageNum) {
        ...
        window.location="/treehole/"+goto_page;
    }
...
<div class="col-xs-9 banner-body-right">
    ...
    <!--添加秘密-->
        ...
        <hr/>
        <div class="row" id="searchSecret">
            <span class="glyphicon glyphicon-search" style="margin: 0 10px"></span>
            <input type="text"/><button class="btn btn-default btn-sm" style="margin: 0 10px">搜索</button>
        </div>
        <hr/>
        <!--查看秘密1-->
    ...
    <ul class="pagination pull-right">
        ...
        <c:forEach begin="${secretPage.start}" end="${secretPage.end}" var="page">
            <c:if test="${secretPage.pageNum==page}">
                <li class="active"><a href="<c:url value="/treehole/${page}"/>" class="">${page}</a></li>
            </c:if>
            <c:if test="${secretPage.pageNum!=page}">
                <li><a href="<c:url value="/treehole/${page}"/>">${page}</a></li>
            </c:if>
        </c:forEach>
