...
<script>
    ...
    function lastPage(pageNum) {
        ...
        window.location="/treehole/search/${search}/"+goto_page;
    }
...
<div class="col-xs-9 banner-body-right">
    <div class="panel-group" id="accordion">
        ...
        <!--搜索结果-->
        <c:if test="${secretPage.totalRecord==0}">
            <div class="alert alert-danger">
                <button type="button" class="close" data-dismiss="alert"
                        aria-hidden="true" onclick="window.location.href='/home'">
                    &times;
                </button>
                没有找到包含<span style="font-weight: bold;font-size: 16px">&nbsp${content}&nbsp</span>内容的秘密！
            </div>
        </c:if>
        <c:if test="${secretPage.totalRecord>0}">
            <div class="alert alert-success alert-dismissable">
                ...
                找到<span style="font-weight: bold;font-size: 16px">&nbsp${secretPage.totalRecord}条&nbsp</span>包含<span style="font-weight: bold;font-size: 16px">&nbsp${content}&nbsp</span>内容的秘密，共<span style="font-weight: bold;font-size: 16px">&nbsp${secretPage.totalPage}页&nbsp</span>
            </div>
        </c:if>
        ...
        <c:forEach items="${secretPage.list}" var="secret">
            <div class="panel panel-default" style="box-shadow: 1px 1px 5px #888888">
                <div class="panel-body">
                        <%--<c:if test="${secret.anonymous==true}"><div class="row">
                            <img src="images/niming.jpg" alt="匿名头像" class="img-responsive img-circle" style="float:left; margin-left:10px" width="50px"/>
                            <span style="margin-left:10px; line-height: 1.2cm ">匿名用户</span></div></c:if>--%>
    ...
    <ul class="pagination pull-right">
        ...
        <c:forEach begin="${secretPage.start}" end="${secretPage.end}" var="page">
            <c:if test="${secretPage.pageNum==page}">
                <li class="active"><a href="<c:url value="/treehole/search/${search}/${page}"/>" class="">${page}</a></li>
            </c:if>
            <c:if test="${secretPage.pageNum!=page}">
                <li><a href="<c:url value="/treehole/search/${search}/${page}"/>">${page}</a></li>
            </c:if>
        </c:forEach>
